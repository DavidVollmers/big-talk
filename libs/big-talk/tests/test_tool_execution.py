...
@pytest.mark.asyncio
async def test_tool_result_serialization(bigtalk, simple_message):
    """
    Verify that Python types (dicts, ints, None) are correctly
    serialized into strings/JSON for the LLM.
    """

    # 1. Define tools with various return types
    async def ret_str():
        return "raw string"
    ...
    # 2. Mock the LLM calling all of them
    tool_msg = AssistantMessage(
        role="assistant",
        content=[
            ToolUse(type="tool_use", id="1", name="ret_str", params={}),
            ToolUse(type="tool_use", id="2", name="ret_int", params={}),
            ToolUse(type="tool_use", id="3", name="ret_float", params={}),
            ToolUse(type="tool_use", id="4", name="ret_bool", params={}),
            ToolUse(type="tool_use", id="5", name="ret_none", params={}),
            ToolUse(type="tool_use", id="6", name="ret_dict", params={}),
            ToolUse(type="tool_use", id="7", name="ret_list", params={}),
        ],
    ...
    # 3. Execute
    history = [simple_message]
    tools = [ret_str, ret_int, ret_float, ret_bool, ret_none, ret_dict, ret_list]

    async for msg in bigtalk.stream("test/model", history, tools=tools):
        if msg['role'] == 'tool':
            history.append(msg)

    # 4. Verification
    results = history[-1]['content']
    res_map = {r['tool_use_id']: r['result'] for r in results}

    # Strings should remain strings (no extra quotes)
    assert res_map['1'] == "raw string"

    # Primitives should be stringified
    assert res_map['2'] == "42"
    assert res_map['3'] == "3.14"

    # Booleans should be lowercased JSON style (if using json.dumps)
    assert res_map['4'] == "true"

    # None should be JSON null
    assert res_map['5'] == "null"

    # Dicts should be valid JSON
    assert '{"key": "value"' in res_map['6']

    # Lists should be valid JSON
    assert '["a", "b"]' in res_map['7']
